    ...
    <Page>
      ...
      <Box style={{ position: "relative", height: 260, overflow: "hidden" }}>
        <img
          ...
          alt={`${room.name} – ${hotel.name} Quy Nhơn`}
        ...
        >
          <Text style={{ color: "rgba(255,255,255,0.7)", fontSize: 11, marginBottom: 4, fontWeight: 500 }}>
            {hotel.name}
          </Text>
      ...
      {room.images && room.images.length > 1 && (
        <Box
          style={{ display: "flex", gap: 6, padding: "10px 16px", overflowX: "auto", WebkitOverflowScrolling: "touch" }}
        >
      ...
      {/* paddingBottom 96 chừa BottomBar */}
      <Box style={{ padding: "16px 16px 96px", display: "flex", flexDirection: "column", gap: 12 }}>
        ...
        <Box style={{ display: "flex", gap: 8 }}>
          {[
            ...
            <Box
              key={spec.label}
              style={{ flex: 1, background: "#F8F9FA", borderRadius: 10, padding: "10px 8px", textAlign: "center" }}
            >
              <Text style={{ fontSize: 18, display: "block", marginBottom: 4 }}>{spec.icon}</Text>
              <Text style={{ fontSize: 13, fontWeight: 700, color: "#1A2535", display: "block" }}>{spec.value}</Text>
        ...
        {/* Mô tả */}
        <Box style={{ background: "#fff", borderRadius: 12, padding: 14, border: "0.5px solid rgba(0,0,0,0.08)" }}>
          <Text style={{ fontSize: 12, color: "#7F8C8D", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8, display: "block" }}>
            Giới thiệu phòng
          </Text>
          <Text style={{ fontSize: 13, color: "#2C3E50", lineHeight: 1.7 }}>{room.excerpt}</Text>
        </Box>

        {/* Tiện ích */}
        <Box style={{ background: "#fff", borderRadius: 12, padding: 14, border: "0.5px solid rgba(0,0,0,0.08)" }}>
          <Text style={{ fontSize: 12, color: "#7F8C8D", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10, display: "block" }}>
            Tiện ích phòng
          </Text>
          <Box style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {room.amenities?.map((a) => (
              <span
                key={a}
                style={{ background: "#F0F8FF", border: "0.5px solid #BDE3F5", borderRadius: 20, fontSize: 11, color: "#1D7FA3", padding: "4px 12px", fontWeight: 500 }}
              >
                {a}
              </span>
            ))}
        ...
        {/* Giá */}
        <Box style={{ background: "#FFFBF0", borderRadius: 12, padding: 14, border: "0.5px solid rgba(201,168,76,0.3)" }}>
          <Text style={{ fontSize: 11, color: "#7F8C8D", marginBottom: 4, display: "block" }}>Giá từ</Text>
